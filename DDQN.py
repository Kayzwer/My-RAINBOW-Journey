...
class Epsilon_Controller:
    ...
    def decay(self, last_reward: int) -> None:
...
class Agent:
    def __init__(
        ...
        self.gamma = gamma
        self.tau = tau
        self.network = DDQN(input_dim, output_dim, learning_rate)
        self.target_network = DDQN(input_dim, output_dim, learning_rate)
    ...
    def learn(self, env: gym.Env) -> float:
        ...
        Argmax_action = torch.argmax(self.network.forward(next_state_batch),
                                     dim=1)
        Q_Target = reward_batch + self.gamma * \
            Q_Next[batch_index, Argmax_action] * ~terminal_state_batch
...
if __name__ == "__main__":
    ...
    agent = Agent(
        ...
        learning_rate=0.001, gamma=0.99, tau=0.05, buffer_size=2048,
        batch_size=1024, epsilon=1.0, epsilon_decay_rate="0.05",
        minimum_epsilon=0.0, reward_target=25, reward_target_grow_rate=25,
        confidence=3)
    iteration = 100
    ...
    for i in range(iteration):
        ...
        print(f"Iteration: {i + 1}, Epsilon: "
              f"{agent.epsilon_controller.epsilon}, Current Target: "
              f"{agent.epsilon_controller.reward_target}, Last Game Score: "
              f"{score}")
